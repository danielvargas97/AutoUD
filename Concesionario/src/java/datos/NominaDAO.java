...
public class NominaDAO {
    ...
    public int valorTotalVentas(int idVendedor) throws CaException {
        int suma = 0;
        //List<Nomina> nominas = new ArrayList<Nomina>();                  
        try {
            String strSQL = "select SUM(precioDeOrden) ";
            strSQL += "from vendedor ve, ordenPago op ";
            strSQL += "where op.idVendedor = ? AND ";
            strSQL += "op.estadoOrden = 'pagado' AND ";
            strSQL += "op.idVendedor = ve.idVendedor";

            Connection conexion = ServiceLocator.getInstance().tomarConexion();
            PreparedStatement prepStmt = conexion.prepareStatement(strSQL);
            prepStmt.setInt(1, idVendedor);
            //prepStmt.setString(1,tipo.getkTipo());
            ResultSet rs = prepStmt.executeQuery();
            while (rs.next()) {
                suma = rs.getInt(1);
